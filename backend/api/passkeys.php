...
if ($method === 'POST') {
    ...
    if ($action === 'register') {
        ...
        // RP-ID bestimmen (server-ready, kein hardcodierter localhost)
        $host = $_SERVER['HTTP_HOST'] ?? $_SERVER['SERVER_NAME'] ?? '';
        if (empty($host)) {
            // Fallback nur wenn wirklich nichts verfügbar
    ...
    if ($action === 'verify') {
        ...
        $passkeyId = $insertStmt->insert_id;
        AuditService::log($mysqli, 'passkey_created', $currentUserId, 'passkey', $passkeyId, [
