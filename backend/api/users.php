...
if ($method === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        try {
            ...
            // 1. Ziel-Fortschritt löschen (SQL Injection Schutz: Prepared Statement)
            $deleteProgressStmt = $mysqli->prepare("DELETE ugp FROM user_goal_progress ugp 
                           INNER JOIN user_goals ug ON ugp.user_goal_id = ug.id 
                           WHERE ug.user_id = ?");
            if ($deleteProgressStmt) {
                $deleteProgressStmt->bind_param('i', $deleteUserId);
            ...
            if ($passkeyDeleteStmt) {
                ...
                @$passkeyDeleteStmt->execute();
            }
            $challengeDeleteStmt = $mysqli->prepare("DELETE FROM passkey_challenges WHERE user_id = ?");
            if ($challengeDeleteStmt) {
                $challengeDeleteStmt->bind_param('i', $deleteUserId);
                @$challengeDeleteStmt->execute();
            }
            
            // 10. Gruppen des Users löschen (created_by)
            // Erst Mitglieder aus diesen Gruppen entfernen (SQL Injection Schutz: Prepared Statement)
            $deleteGroupMembersStmt = $mysqli->prepare("DELETE gm FROM group_members gm 
                           INNER JOIN student_groups sg ON gm.group_id = sg.id 
                           WHERE sg.created_by = ?");
            if ($deleteGroupMembersStmt) {
                $deleteGroupMembersStmt->bind_param('i', $deleteUserId);
            ...
            AuditService::log($mysqli, 'user_deleted', $currentUserId, 'user', $deleteUserId, [
    ...
    if ($action === 'changePassword') {
        ...
        $passwordValidation = validate_password_length($mysqli, $body['newPassword']);
        ...
        AuditService::log($mysqli, 'password_changed', $currentUserId, 'user', (int)$body['id'], ['target_user_id' => (int)$body['id']]);
    ...
    if ($action === 'changeOwnPassword') {
        ...
        AuditService::log($mysqli, 'password_changed', $currentUserId, 'user', $currentUserId);
        json_out(['success'=>true, 'message'=>'Passwort erfolgreich geändert']);
    }
...
if ($method === 'PUT') {
    ...
    if ($action === 'updateProfile') {
        ...
        if (!$stmt->execute()) {
            // Error Leakage verhindern: Keine DB-Fehlermeldungen an Client
            json_out(['success'=>false, 'error'=>'Profil-Update fehlgeschlagen'], 500);
        }
